...
import type { AgentCard, ModelProviderEnum, ProviderInfo, ProviderSettings } from 'src/shared/types'
import { z } from 'zod'
...
import { AgentProviderList } from '@/components/settings/agent-provider/AgentProviderList'
import { AddAgentProviderModal } from '@/components/settings/agent-provider/AddAgentProviderModal'
...
export const Route = createFileRoute('/settings/agent_provider')({
...
function RouteComponent() {
  ...
  // )
  const agentProviders = useMemo<AgentCard[]>(
    () =>
      [...(settings.agentProviders || [])]
    , [settings.agentProviders]
  )
  const [newAgentProviderModalOpened, setNewAgentProviderModalOpened] = useState(false);
  ...
  // }
  const openModal = () => {
    setNewAgentProviderModalOpened(true);
  } 
  return (
    <Flex h="100%" w="100%">
      {/* {(!isSmallScreen || routerState.location.pathname === '/settings/agent_provider') && (
        <ProviderList
          providers={providers}
      ...
      <AddAgentProviderModal opened={newAgentProviderModalOpened} onClose={() => setNewAgentProviderModalOpened(false)}></AddAgentProviderModal>
    </Flex>
